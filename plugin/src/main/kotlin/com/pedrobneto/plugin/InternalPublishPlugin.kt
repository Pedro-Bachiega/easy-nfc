...
import com.pedrobneto.plugin.util.missing
import com.pedrobneto.plugin.util.publishing
import com.pedrobneto.plugin.util.requireAny
import com.pedrobneto.plugin.util.setupJavadocAndSources
import com.pedrobneto.plugin.util.sign
import com.pedrobneto.plugin.util.versionName
...
internal class InternalPublishPlugin : Plugin<Project> {
    ...
    override fun apply(target: Project) {
        target.requireAny(
            ...
            "internal-multiplatform-library"
        )
        ...
        with(target.publishing) {
            repositories { repo ->
                ...
                repo.createSonatypeReleaseRepository(target)
                repo.createSonatypeSnapshotRepository(target)
            }

            publications {
                it.register("Lib", MavenPublication::class.java) { pub ->
    ...
    private fun RepositoryHandler.createSonatypeRepository(
        project: Project,
        repositoryType: String,
        url: String
    ) = maven { maven ->
        maven.name = "Sonatype$repositoryType"
        maven.url = project.uri(url)
        maven.credentials { cred ->
            cred.username = System.getenv("OSSRH_USERNAME")
                ?: (project.properties["OSSRH_USERNAME"] as String)
            cred.password = System.getenv("OSSRH_PASSWORD")
                ?: (project.properties["OSSRH_PASSWORD"] as String)
        }
    }

    private fun RepositoryHandler.createSonatypeReleaseRepository(project: Project) =
    ...
    private fun RepositoryHandler.createSonatypeSnapshotRepository(project: Project) =
        createSonatypeRepository(
            project,
            "Snapshot",
